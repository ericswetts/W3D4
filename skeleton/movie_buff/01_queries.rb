...
def harrison_ford
  ...
  Movie.joins(:actors, :castings).where(actors: { name: "Harrison Ford" }).select(:id, :title).where("castings.ord > 1").order(:id).distinct
...
def biggest_cast
  ...
  Movie.joins(:actors).group(:id).order('COUNT(*) DESC').limit(3).select(:id, :title)
...
def directed_by_one_of(them)
  ...
  # " WHERE actors.name IN [name1, name2]"
end
